...
for i in range(1, Nt + 1):
    ...
    w2 = l21*Pint + l22*Qint
    w1 = (inflow(time[i]) - r22*w2)/r21
    Qomega = r21*w1 + r22*w2
    Pomega = r11*w1 + r12*w2
    u[1,0] = Qomega #nflow(time[i])
    u[0,0] = Pomega #(inflow(time[i])-Qint)*r11+Pint
    ...
    Pint = np.interp(xint, x, un[i-1,0,:])
    Qint = np.interp(xint, x, un[i-1,1,:])
    w1 = l11*Pint + l12*Qint
    dw1 = (w1 - l11*PoutletLast - l12*QoutletLast)
    w2 = w1*reflectionCoefficient
    dw2 = dw1*reflectionCoefficient
    Pnew = r11*w1 + r12*w2
    Qnew = r21*w1 + r22*w2
    
    Pnew = PoutletLast + r11*dw1 + r12*dw2
    Qnew = QoutletLast + r21*dw1 + r22*dw2
    


    u[0, -1] = Pnew #np.interp(xint, x, un[i - 1, 0, :]) #calculate outlet boundary [P]from wavespeed and interpolate
    u[1, -1] = Qnew #np.interp(xint, x, un[i - 1, 1, :]) #calculate outlet boundary [Q]from wavespeed and interpolate
